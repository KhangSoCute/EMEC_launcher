...
class SampleGUI:
    ...
    def after_have_data(self,data):
        ...
        new_ws['A1'] = 'No.Sample'
        new_ws['A2'] = num_sample
        
        for i in range(num_sample):
            new_ws.cell(row=1,column=i+2,value=f"No.{i+1}")
            new_ws.cell(row=2,column=i+2,value=arr_barcord[i])

        new_wb.save(new_filename)
        
        excel = win32.gencache.EnsureDispatch('Excel.Application')
        wb = excel.Workbooks.Open(os.path.abspath(new_filename))
        new_filename_xls = 'input.xls'
        ...
        messagebox.showinfo("Succeed","Created input file!")
    ...
    def insert_to_template(self,num_sample,data,protocol):
        try:
            ...
            messagebox.showerror("Error", "length of data not equal number of sample, can not create .plnr")
            return
        ...
        if protocol == "HBV":
            ...
            now = datetime.now()
            new_filename = filename_base + '-' + f'{now.day}_{now.month}_{now.year}-{now.hour}_{now.minute}_{now.second}' + new_extension
             
            
            # Rename the file
            shutil.copyfile('LIMS_HBV_output.csv', 'C:\\Seegene_Method_Setting\\plrn\\'+new_filename)
        ...
        if protocol == "HCV":
            ...
            # Rename the file
            shutil.copyfile('LIMS_HCV_output.csv', 'C:\\Seegene_Method_Setting\\plrn\\' + new_filename)
        ...
        if protocol == "HIV":
            ...
            # Rename the file
            shutil.copyfile('LIMS_HIV_output.csv', 'C:\\Seegene_Method_Setting\\plrn\\' + new_filename)
    ...
    def choose_med_file(self):
        ...
        def run_med_file(text_box):
            ...
            # print(int(self.num_samples_entry.get()))
            # print(self.data)
            # print(selected_protocol.get())
